...
public abstract class BaseScaleScrollView extends ViewGroup {
    protected int scaleColor = Color.parseColor("#c8c8c8");
    protected float scaleTextSize = 20;
    protected int scaleWidth = (int) DensityUtil.dp2px(getContext(), 12);//刻度间隔
    protected int scaleMaxHeight = (int) DensityUtil.dp2px(getContext(), 18);//刻度高度（最长的）
    protected float minScale = 0;//最小刻度
    protected float maxScale = 1000;//最大刻度
    protected int cardViewPandding = 1;
    protected boolean scaleShowUp = true;//尺子默认向上
    protected boolean scaleShowShortUp = true;//刻度线默认顶部对齐
    ...
    public BaseScaleScrollView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.ScaleScrollView, defStyleAttr, 0);
        ...
        for (int i = 0; i < n; i++) {
            ...
            } else if (attr == R.styleable.ScaleScrollView_ScaleShowShortUp) {
                scaleShowShortUp = a.getBoolean(attr, scaleShowShortUp);
            } else {
            }
        }
