...
public class ScaleView extends View {
    ...
    private int scaleWidth = 30;//刻度间隔
    private int scaleMinHeight = 22;//小刻度高度
    private int scaleMaxHeight = 44;//大刻度高度
    ...
    private int textColor = Color.parseColor("#c8c8c8");
    private int color1 = Color.parseColor("#ffffff");//Color.rgb(245, 246, 249);//背景圆颜色 红
    private int color2 = Color.parseColor("#ffffff");//背景圆颜色  黄
    private int color3 = Color.parseColor("#ffffff");//背景圆颜色  绿
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Log.e("SJL", "height----" + height + "------" + viewHigth + "-----" + "scaleMaxHeight" + scaleMaxHeight);
        ...
        for (int i = 0; i <= max; i++) {
            ...
            if (isShowUp) {
                if (scaleShowShortUp)
                    ...
                    float currtenStartY = startY;
                    if (Math.abs((yScale - scaleMaxHeight)) != 0) {
                        currtenStartY = scaleMaxHeight - yScale;
                    }
                    canvas.drawLine(xScale, currtenStartY, xScale, scaleMaxHeight, mPaint);
                }
    ...
    private boolean scaleShowShortUp = true;
    public void setScaleShowShortUp(boolean scaleShowShortUp) {
        this.scaleShowShortUp = scaleShowShortUp;
        invalidate();
    }
    ...
    public void computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            ...
            if (mScrollListener != null) {
                if (isIntegar) {
                    mScrollListener.onScaleScroll((int) (thisScale = tmpCountScale + minScale));
                } else {
                    mScrollListener.onScaleScroll(thisScale = new BigDecimal((float) (tmpCountScale) / 10 + minScale).setScale(1, BigDecimal.ROUND_HALF_UP).floatValue());
                }
            }
        }
    }
    ...
    public float getThisScale() {
        return thisScale == 0.0f ? minScale : thisScale;
    }
    ...
    public int getColorChanges(int cl1, int cl2, float runNum) {
