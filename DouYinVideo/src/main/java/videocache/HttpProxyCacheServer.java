...
import static videocache.HttpProxyPreLoader.preUrlPx;
...
public class HttpProxyCacheServer {
    ...
    private HttpProxyPreLoader preLoader;
    ...
    private HttpProxyCacheServer(Config config) {
        preLoader = new HttpProxyPreLoader();
        ...
        try {
            ...
            LogU.d("Proxy cache server started. Is it alive? " + isAlive());
    ...
    public void registerCacheListener(CacheListener cacheListener, String url) {
        ...
        synchronized (clientsLock) {
            try {
              ...
              LogU.d("Error registering cache listener", e);
            }
        }
    }
    ...
    private void touchFileSafely(File cacheFile) {
        try {
            ...
            LogU.e("Error touching file " + cacheFile, e);
        }
    }
    ...
    private void waitForRequest() {
        try {
            while (!Thread.currentThread().isInterrupted()) {
                ...
                LogU.d("Accept new socket " + socket);
    ...
    private void processSocket(Socket socket) {
        try {
            ...
            String originUrl = request.uri;
            String proxyUrl = originUrl;
            boolean isPreLoad = false;
            if (originUrl.contains(preUrlPx)) {
                //预下载
                isPreLoad = true;
                proxyUrl = originUrl.split(preUrlPx)[0];
            }

            if (isPreLoad) {
                request.isPreLoad = true;
                request.percentsPreLoad = Integer.parseInt(originUrl.split(preUrlPx)[1].split("_")[1]);
            } else {
                request.isPreLoad = false;
                if (preLoader.isLoading(originUrl)) {
                    preLoader.stopLoad(originUrl);
                }
            }

            LogU.d("Request to cache proxy:" + request);
            String url = ProxyCacheUtils.decode(proxyUrl);
            ...
            LogU.d("Closing socket… Socket is closed by client.");
        } catch (ProxyCacheException | IOException e) {
            onError(new ProxyCacheException("Error processing request", e));
        } finally {
            releaseSocket(socket);
            LogU.d("Opened connections: " + getClientsCount());
        }
    }
    ...
    public boolean preLoad(final String url, final int percentsPreLoad) {
        final String proxyUrlOrigin = getProxyUrl(url, false);
        final String proxyUrl = proxyUrlOrigin + preUrlPx + "_" + percentsPreLoad;

        if (preLoader.isLoading(proxyUrl)) {
            return false;
        } else {
            //      preLoader.startLoad(proxyUrl, percentsPreLoad, Math.abs(21474836));
            preLoader.cachedThreadPool.execute(new Runnable() {
                @Override
                public void run() {
                    HttpProxyCacheServerClients clients = null;
                    try {
                        ...
                        if (!cacheProxy.cache.isCompleted()) {
                            ...
                            LogU.d("预加载文件大小" + length+" 本地缓存大小　"+cacheLen+ "  "+(cacheLen < Math.abs(length) * (percentsPreLoad / 100.0)));
                            if (cacheLen < Math.abs(length) * (percentsPreLoad / 100.0)) {
                                preLoader.startLoad(proxyUrl, percentsPreLoad, Math.abs(length));
                            }
                        }
                    } catch (ProxyCacheException e) {
                        e.printStackTrace();
    ...
    private void closeSocketInput(Socket socket) {
        try {
            ...
            // There is no way to determine that client closed connection http://stackoverflow.com/a/10241044/999458
            // So just to prevent log flooding don't log stacktrace
            LogU.d("Releasing input stream… Socket is closed by client.");
        } catch (IOException e) {
    ...
    private void closeSocketOutput(Socket socket) {
        try {
          ...
          LogU.d("Failed to close socket on proxy side: {}. It seems client have already closed connection.", e);
        }
    }
    ...
    private void onError(Throwable e) {
        LogU.e("HttpProxyCacheServer error", e);
    }
