...
//        mClient = new OkHttpClient.Builder().build();
        mClient = OkHttpManager.getInstance().client();
    }
     ...
     *
     * @param url              下载请求的网址
     * @param toFilePath       下载路径
     * @param downLoadObserver 用来回调的接口
     */
    public void download(String url, String toFilePath, DownLoadObserver downLoadObserver) {
        Observable.just(url)
                .filter(s -> !downCalls.containsKey(s))//call的map已经有了,就证明正在下载,则这次不下载
                .flatMap(s -> Observable.just(createDownInfo(s, toFilePath)))
     ...
     * @return DownInfo
     */
    private DownloadInfo createDownInfo(String url, String toFilePath) {
        DownloadInfo downloadInfo = new DownloadInfo(url);
        long contentLength = getContentLength(url);//获得文件大小
        downloadInfo.setFilePath(toFilePath);
