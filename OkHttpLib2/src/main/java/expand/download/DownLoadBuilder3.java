    ...
    private class DownloadTask extends AsyncTask<String, Void, Integer> {
        ...
        protected Integer doInBackground(String... strings) {
            ...
            Request req = new Request.Builder()
                    ...
                    .build();
            try {
                Response response = OkHttpFactory.getInstance()
                        .client()
                        .newBuilder()
...
//                        listener.complete(info.file);
        ...
        private long getContentLength(String downloadUrl, DownLoadListenerAdapter listener) {
            ...
            try {
                Response response = OkHttpFactory.getInstance().client().newCall(request).execute();
                if (response != null && response.isSuccessful()) {
    ...
    private void clear(Object tag, boolean cancelAll) {
        for (Call call : OkHttpFactory.getInstance().client().dispatcher().queuedCalls()) {
            if (!cancelAll) {
        ...
        for (Call call : OkHttpFactory.getInstance().client().dispatcher().runningCalls()) {
            if (!cancelAll) {
                if (tag != null && tag.equals(call.request().tag())) {
                    call.cancel();
                }
